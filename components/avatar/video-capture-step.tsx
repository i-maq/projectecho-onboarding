...
export function VideoCaptureStep({ personalData, onComplete, onBack, onSkip }: VideoCaptureStepProps) {
  ...
  const waitForVideoReady = (videoElement: HTMLVideoElement): Promise<void> => {
    return new Promise((resolve, reject) => {
      ...
      const checkDimensions = () => {
        ...
        else setTimeout(checkDimensions, 100);
      };
      checkDimensions();
    });
  };
  ...
  const startCamera = useCallback(async () => {
    try {
      ...
      if (!videoRef.current) { setError('Video element not ready. Please try again.'); stream.getTracks().forEach(t => t.stop()); return; }
      ...
      video.onloadedmetadata = async () => {
        try {
          ...
          try { await waitForVideoReady(video); setIsCameraActive(true); toast.info('Click the video area if camera preview doesn\'t appear'); }
          catch (dimensionError) { setError('Camera failed to initialize properly. Please try again.'); }
        }
      };

      video.onerror = () => setError('Error loading camera feed');
    ...
    try {
      ...
      mediaRecorder.onstop = () => {
        ...
        if (previewRef.current) {
          ...
          previewRef.current.onloadedmetadata = () => previewRef.current?.play().catch(console.error);
        }
      };
      mediaRecorder.start();
      setIsRecording(true);
      setRecordingTime(0);
      const intervalId = setInterval(() => setRecordingTime(prev => prev + 1), 1000);
      setTimeout(() => { if (mediaRecorder.state !== 'inactive') { clearInterval(intervalId); mediaRecorder.stop(); } }, 30000);
      return () => clearInterval(intervalId);
    } catch (error) { setError('Failed to start recording. Please try again.'); }
  }, []);

  const stopRecording = useCallback(() => {
    if (!streamRef.current) return;
    try {
      streamRef.current.getTracks().forEach(track => { if (track.readyState === 'live' && track.kind === 'video') track.stop(); });
      setIsRecording(false);
    } catch (err) { setError('Failed to stop recording. Please try again.'); }
  }, []);
  ...
  const startCountdown = useCallback(() => {
    ...
    const countdownInterval = setInterval(() => {
      setCountdown(prev => {
        if (prev === 1) { clearInterval(countdownInterval); startRecording(); return null; }
  ...
  const restartCapture = useCallback(() => {
    setRecordedVideo(null); setIsPlaying(false); setReplicaId(null); setReplicaStatus(null);
  ...
  const playPausePreview = useCallback(() => {
    if (previewRef.current) {
      ...
      else previewRef.current.play().catch(console.error);
  ...
  const uploadVideoToTavus = async () => {
    ...
    try {
      ...
      if (!token) { toast.error('Authentication required. Please sign in again.'); setIsProcessing(false); return; }

      const response = await fetch('/api/tavus/replica', { method: 'POST', headers: { 'Authorization': `Bearer ${token}` }, body: formData });
      if (!response.ok) { const errorData = await response.json(); throw new Error(errorData.error || 'Failed to create replica'); }
      const replicaData = await response.json();
      setReplicaId(replicaData.id);
      setReplicaStatus(replicaData.status);
      const interval = setInterval(async () => { await checkReplicaStatus(replicaData.id); }, 5000);
      setStatusCheckInterval(interval);
      toast.success('Video uploaded successfully! Your Echo avatar is being created.');
    } catch (error) { toast.error('Failed to upload video. Please try again.'); }
  ...
  const checkReplicaStatus = async (id: string) => {
    try {
      ...
      const response = await fetch(`/api/tavus/replica?id=${id}`, { headers: { 'Authorization': `Bearer ${token}` } });
      if (!response.ok) throw new Error('Failed to check replica status');
      const replicaData = await response.json();
      setReplicaStatus(replicaData.status);
    ...
    } catch (error) { console.error('Error checking replica status:', error); }
  };
  ...
  useEffect(() => {
    return () => {
      ...
      if (previewRef.current && previewRef.current.src.startsWith('blob:')) URL.revokeObjectURL(previewRef.current.src);
    };
      ...
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.6 }} className="w-full max-w-2xl mx-auto">
        <div className="glass-panel-light text-center">
          <motion.div className="text-center mb-6" initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3, duration: 0.6 }}>
            <div className="w-20 h-20 bg-gradient-to-br from-sky-400 to-teal-500 rounded-2xl flex items-center justify-center shadow-lg mx-auto">
              <Video className="h-10 w-10 text-white" />
            </div>
            <h2 className="text-3xl font-bold mt-4 mb-3 text-gray-800 text-title">Create Your Echo Avatar</h2>
            ...
            {isCameraActive && !recordedVideo && (
              <div className="rounded-lg p-4 mb-6 text-left" style={{ background: 'rgba(14, 165, 233, 0.08)', border: '1px solid rgba(14, 165, 233, 0.18)' }}>
                <h3 className="font-semibold text-sky-800 mb-2">Please read aloud:</h3>
                <p className="font-medium text-sky-800 text-lg">"{scriptPrompts[currentScript]}"</p>
                {currentScript < scriptPrompts.length - 1 && (
                  <p className="text-sm text-sky-600 mt-2">{currentScript + 1} of {scriptPrompts.length} phrases</p>
                )}
              </div>
            )}

            {replicaId && !replicaStatus?.includes('ready') && (
              <div className="bg-sky-100 border border-sky-200 rounded-lg p-4 mb-6">
                <div className="flex items-center">
                  <Loader2 className="h-5 w-5 text-sky-500 animate-spin mr-2" />
                  <h3 className="font-semibold text-sky-800">Creating Your Echo Avatar ({replicaStatus || 'initializing'}...)</h3>
                </div>
                <p className="text-sm text-sky-700 mt-2">This process typically takes 10-15 minutes. You can continue using the app and we'll notify you when it's ready.</p>
              </div>
            )}
            ...
            {replicaStatus === 'ready' && (
              <div className="bg-emerald-100 border border-emerald-200 rounded-lg p-4 mb-6">
                <div className="flex items-center">
                  ...
                  <h3 className="font-semibold text-emerald-800">Your Echo Avatar is Ready!</h3>
                </div>
                <p className="text-sm text-emerald-700 mt-2">Proceeding to your dashboard where you can interact with your Echo.</p>
              </div>
            )}
          </motion.div>

          <div className="relative">
            <div className={isCameraActive && !recordedVideo ? 'space-y-6' : 'invisible h-0 overflow-hidden'} style={{ position: 'relative', zIndex: 2 }}>
              <div className="relative">
                {countdown !== null && (
                  <div className="absolute inset-0 flex items-center justify-center bg-black bg-opacity-40 z-10 rounded-2xl">
                    <div className="bg-white rounded-full h-24 w-24 flex items-center justify-center">
                      <span className="text-5xl font-bold text-sky-600">{countdown}</span>
                    </div>
                  </div>
                )}
                <video ref={videoRef} autoPlay muted={isMuted} playsInline className="w-full h-80 bg-black rounded-2xl mx-auto object-cover" />
                {isRecording && (
            ...
            {!isCameraActive && !recordedVideo && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
                ...
                <div className="text-center">
                  <button onClick={startCamera} disabled={isProcessing} className="glass-button glass-button-primary text-button px-8 py-3 disabled:opacity-50">
                    <Camera className="h-5 w-5 mr-2 inline" />Start Camera
                  </button>
                </div>
              </motion.div>
            )}
            ...
            {recordedVideo && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
                <div className="relative">
                  <video ref={previewRef} controls className="w-full h-80 bg-black rounded-2xl mx-auto object-cover" />
                </div>
