...
export function EchoAvatarPlayer({ replicaId, prompt, autoplay = false, className = '' }: EchoAvatarPlayerProps) {
  ...
  useEffect(() => {
    if (prompt && prompt.trim()) generateVideo(prompt);
  }, [prompt]);

  useEffect(() => {
    return () => { if (statusCheckInterval) clearInterval(statusCheckInterval); };
  }, [statusCheckInterval]);
        ...
        {isLoading ? (
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="w-full h-full">
            ...
            <div className="absolute bottom-0 left-0 right-0 p-3 bg-gradient-to-t from-black/70 to-transparent flex justify-between items-center">
              <button onClick={togglePlayPause} className="text-white hover:text-sky-300">
                {isPlaying ? <Pause className="h-6 w-6" /> : <Play className="h-6 w-6" />}
              </button>

              <div className="flex space-x-3">
                <button onClick={toggleMute} className="text-white hover:text-sky-300">
                  {isMuted ? <VolumeX className="h-5 w-5" /> : <Volume2 className="h-5 w-5" />}
                </button>
                <button onClick={handleRegenerate} className="text-white hover:text-sky-300">
